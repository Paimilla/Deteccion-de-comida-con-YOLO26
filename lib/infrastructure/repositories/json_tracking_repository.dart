...
class JsonTrackingRepository implements TrackingRepository {
  ...
  Future<List<DiaryEntry>> getEntriesBetween(DateTime from, DateTime to) async {
    ...
    return entries
        ...
        .where(
          (entry) =>
              !entry.timestamp.isBefore(fromDay) &&
  ...
  Map<String, dynamic> _entryToJson(DiaryEntry entry) {
    return {
      ...
      'food': {
        ...
        'portion': {
          'amount': entry.food.portion.amount,
          'unit': entry.food.portion.unit,
        },
        'nutrition': {
  ...
  DiaryEntry _entryFromJson(Map<String, dynamic> json) {
    final food = (json['food'] as Map?)?.cast<String, dynamic>() ??
        <String, dynamic>{};
    final portion =
        (food['portion'] as Map?)?.cast<String, dynamic>() ??
            <String, dynamic>{};
    final nutrition =
        (food['nutrition'] as Map?)?.cast<String, dynamic>() ??
            <String, dynamic>{};

    final sourceName = food['source']?.toString() ?? FoodSource.unknown.name;
    final source = FoodSource.values.firstWhere(
      (s) => s.name == sourceName,
      orElse: () => FoodSource.unknown,
    );

    final mealSlotName = json['meal_slot']?.toString() ?? MealSlot.snack.name;
    final mealSlot = MealSlot.values.firstWhere(
      (m) => m.name == mealSlotName,
    ...
    return DiaryEntry(
      ...
      food: FoodItem(
        ...
        portion: Portion(
          amount: _asDouble(portion['amount']) ?? 100,
          unit: portion['unit']?.toString() ?? 'g',
        ),
        nutrition: Nutrition(
